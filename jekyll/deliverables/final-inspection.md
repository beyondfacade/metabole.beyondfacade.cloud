...
Sprint 4(10.01~10.08) 검수 단계의 기준 문서다. 시나리오 테스트 전/후 고객 환경에 구축/이전할지는 선택사항(수업 지침)이며, 본 프로젝트는 **로컬 시연을 기본, AWS 이전을 가정 시나리오**로 준비한다.
...
수집 크론(일 1회) → 원천 적재(BC 11개) → 지표 집계 → RAG 색인 크론(05:50) → FastAPI 서빙(REST+SSE) → Next.js 웹. 전체 흐름도는 [상위 설계서 Data Flow]({{ '/docs/deliverables/high-level-design.html' | relative_url }}) 참조.
...
이전 시 데이터는 `pg_dump`/`pg_restore`로 이관하며, 벡터 컬럼 포함 전체가 표준 덤프로 이전된다(재색인 불필요). 개발 ENV·OSS 라이브러리·Git·AI Model 의존은 requirements/lock 파일로 고정되어 있어 재현 가능하다.

### 예외처리

운영 중 예외(외부 API 실패·LLM 타임아웃·부분 적재 실패)의 처리 원칙은 [시나리오 테스트의 예외처리 표]({{ '/docs/deliverables/test-scenario.html' | relative_url }})를 검수 기준으로 사용한다.
...
- 업종 추가: 설정(업종코드·데이터소스 매핑) 추가만으로 확장 — `if 업종` 분기 금지 원칙([개발 표준]({{ '/docs/guidelines/standards.html' | relative_url }}))

## 테스트 시나리오 / 시나리오 상세

[시나리오 테스트]({{ '/docs/deliverables/test-scenario.html' | relative_url }}) 산출물의 시나리오 1~4 + 성능(TPS·Recall@5) 측정을 검수 항목으로 재사용한다. 완료 기준(Definition of Done)은 코드 리뷰 승인 + 테스트 통과 + 문서 반영.

## 유지보수 방안

- **데이터 신선도**: 수집·색인 크론이 무인 갱신. 실패는 `logs/*.log`로 추적.
- **버전 관리**: 백엔드/프론트 각각 `ver_log`에 버전 단위 기록 — 회귀 시 원인 버전 특정 가능.
- **스키마 변경**: Alembic 마이그레이션으로만 수행 (수기 DDL 금지).
- **평가 회귀**: 릴리스 전 Recall@5 하네스 재실행으로 검색 품질 회귀 감지.
- **문서**: 본 사이트가 유지보수 기준 문서이며, 구현 변경은 [개발 일지]({{ '/docs/devlog.html' | relative_url }})에 계속 기록한다.
